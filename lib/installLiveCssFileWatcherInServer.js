...
module.exports = function (app, options, sio) {
    ...
    io.sockets.on('connection', function (client) {
        client.on('watch', function (assetUrls, pageUrl) {
            client.baseDir = options.documentRoot.replace(/\/$/, "") + path.dirname(URL.parse(pageUrl).pathname);
            assetUrls.forEach(function (assetUrl) {
                var rootRelativePath = URL.parse(URL.resolve(pageUrl, assetUrl)).pathname,
                    fileName = path.resolve(options.documentRoot, rootRelativePath.substr(1));
