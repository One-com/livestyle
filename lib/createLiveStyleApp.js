...
module.exports = function createLiveStyleApp(options) {
    ...
    if (options.processImage) {
        app.use(require('express-processimage')({root: options.root}));
    }
    ...
    if (options.autoprefixer) {
        ...
        app.use(require('express-autoprefixer')(autoprefixerOptions));
    }

    if (options.root) {
        if (options.compiless) {
            app.use(require('express-compiless')({root: options.root}));
        }

        if (options.compilesass) {
            app.use(require('express-compile-sass')({
                root: options.root,
                sourceMap: true,
