                    ...
                    upstreamRequest = request({
                        ...
                        if (err) {
                            ...
                            if (response.headers.location && /^https?:/.test(response.headers.location)) {
                                // HTTP redirect. If it's an absolute url pointing at the upstream server, rewrite it to relative one
                                // so it points at the LiveStyle proxy. Technically this is a violation of the HTTP spec, but getting the
                                // LiveStyle servers host/port is a bit hard from here, and browsers understand it fine:
